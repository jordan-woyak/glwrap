...
int main() {
  ...
  auto model_uni =
      vshad.create_uniform(gl::variable<gl::mat4>("model", uniforms));
  auto projection_uni =
      vshad.create_uniform(gl::variable<gl::mat4>("projection", uniforms));

  auto light_dir_uni =
      vshad.create_uniform(gl::variable<gl::vec3>("light_dir", uniforms));
  auto ambient_uni =
      vshad.create_uniform(gl::variable<gl::vec4>("ambient", uniforms));

  auto pos_attrib = vshad.create_input(gl::variable<gl::vec3>("pos", attribs));
  auto norm_attrib =
      vshad.create_input(gl::variable<gl::vec3>("norm", attribs));

  vshad.set_source("out vec3 vertex_normal, norm_light_dir, Ia, E;"
  ...
  auto diff_color_uni =
      fshad.create_uniform(gl::variable<gl::vec4>("diff_color", uniforms));
  auto spec_color_uni =
      fshad.create_uniform(gl::variable<gl::vec4>("spec_color", uniforms));
  auto mat_color_uni =
      fshad.create_uniform(gl::variable<gl::vec4>("mat_color", uniforms));
  auto shininess_uni =
      fshad.create_uniform(gl::variable<gl::float_t>("shininess", uniforms));

  auto fragdata =
      fshad.create_output(gl::variable<gl::vec4>("fragdata", fragdatas));
  (void)fragdata;
  ...
  fshad.set_source(
      ...
      "vec3 Is = spec_color.rgb * spec_color.a * pow(max(dot(R, E), 0.0), "
      "shininess);"
  ...
  // automatically set data types, sizes and strides to components of custom
  // vertex type
  ...
  auto const post_rotate =
      gl::translate(0.1f, -0.8f, 0.f) * gl::scale(8.f, 8.f, 8.f);

  auto const proj =
      gl::perspective(glm::radians(45.f),
                      (float_t)window_size.x / window_size.y, 1.f, 100.f) *
      glm::lookAt(glm::vec3{0.f, 0.f, 3.f}, glm::vec3{},
  ...
  dsp.set_display_func([&] {
    // rotating model
    gl::mat4 model =
        pre_rotate * gl::rotate(rotate, 0.f, 1.f, 0.f) * post_rotate;
